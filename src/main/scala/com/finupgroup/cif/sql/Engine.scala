...
object Engine {
  ...
  implicit class dbTable(table: Table) {
    ...
    def where(where: Option[SqlExpr]): Table = {
      where match {
        case None => table
        case Some(x: SqlExpr) =>println("where"); table filter (evalWhereEachRow(_, x))
      }
    }
  ...
  def execute(table: Table, sql: SelectStmt): Table = {
    sql match {
      //table.from(f).where(w).groupby(g).aggre(s).select(s).orderBy(o).limit(l)
    ...
    (expr.sqlProj, expr.alias) match {
      ...
      case (f: FieldIdent, alias) => alias match {
        case None => {
          ...
          //case (f.name, _2) => (f.name+":33333", _2)
        }
...
//  def kankan(expr: Seq[Projection]) ={
//    expr.map{ x=>
//      case Projection(x) => kankan(x)
//      case
//    }
//  }
  ...
  def selectEachRow(row: Row, expr: Seq[Projection]): Row = {
    ...
    //println("row:::"+row)
    var exmap = expr.map{
      case Projection(fi,_) => fi match {
        case FieldIdent(_,name) => (name->null)
        //case StarProj() =>  return List("name","age","set","birthDay").map(name => (name -> null))
        case StarProj() => ("all"->"all")
        case _ => (""->"")
      }
    }

    if(exmap.head._1 == "all"){
      exmap = List(("name"->null),("age"->null),("sex"->null),("birthDay"->null))
    }

    import collection.{breakOut, mutable}
    val lb = new scala.collection.mutable.ListBuffer[(String,String)]
    val initmap = (lb ++= exmap).toMap
    //val chmap = initmap.asInstanceOf[scala.collection.mutable.Map[String,String]]
    val chmap: mutable.Map[String, String] = initmap.map(identity)(breakOut)
    //println("initmap:::"+initmap)

    //println("exmap:::"+exmap)
    val result = expr map (e => row collect (satisfy(e)))
    //println("result:::"+result.reduce(_ ++ _))
    val remap = result.reduce(_ ++ _)
    val res = initmap ++ remap
    //val res = (chmap.remove("") ++ remap)
    res.asInstanceOf[Row]
  }
